...
ggplot(norm_df, aes(x = SMA, colour = imageID)) +
  geom_density() +
  theme(legend.position = "none")
```
...
cells$cellType <- cells |>
  ...
  mutate(cellType = factor(case_when(
      clusters == "cluster_1" ~ "CK_low_tumour",
      clusters == "cluster_2" ~ "Luminal",
      clusters == "cluster_3" ~ "T_cell_dn",
      clusters == "cluster_4" ~ "T_cell_CD8",
      clusters == "cluster_5" ~ "T_cell_CD4",
      clusters == "cluster_6" ~ "Granulocytes",
      clusters == "cluster_7" ~ "B_cells",
      clusters == "cluster_8" ~ "DC",
      clusters == "cluster_9" ~ "Myofibroblasts",
      clusters == "cluster_10" ~ "Monocyte",
      clusters == "cluster_11" ~ "Monocyte",
      clusters == "cluster_12" ~ "DC",
      clusters == "cluster_13" ~ "ImmuneOther",
      clusters == "cluster_14" ~ "CAF",
      clusters == "cluster_15" ~ "APC",
      clusters == "cluster_16" ~ "Macrophage",
      clusters == "cluster_17" ~ "EMT",
      clusters == "cluster_18" ~ "Mast",
      clusters == "cluster_19" ~ "Endothelial",
      clusters == "cluster_20" ~ "Basal",
      clusters == "cluster_21" ~ "Myoepithelial",
      TRUE ~ clusters
  ))) |> 
  pull(cellType) 
...
cells |> 
  join_features(features = c("FOXP3"), assay = "norm", shape = "wide") |>
  ggplot(aes(x = cellType, y = FOXP3)) + geom_boxplot()
...
prop <- getProp(cells[,cells$imageID%in%imagesToUse], feature = "cellType")
...
treekorParents <- list(Tumour = c("Luminal", "Basal", "EMT", "CK_low_tumour"), T_cell = c("T_cell_dn", "T_cell_CD8", "T_cell_CD4"))
...
```{r}
kontextResults[grep("T_cell_CD8__CK_low_tumour_",rownames(kontextResults)),]

kontextResults[grep("CK_low_tumour__T_cell_CD8_",rownames(kontextResults)),]

kontextResults[grep("T_cell_CD8__T_cell",rownames(kontextResults)),]


boxplot(kontextMat[rownames(prop), "T_cell_CD8__CK_low_tumour__Tumour"]~ droplevels(clinical[rownames(prop), "Status"]))


boxplot(kontextMat[rownames(prop), "CK_low_tumour__T_cell_CD8__T_cell"]~ droplevels(clinical[rownames(prop), "Status"]))
...
regionMap(cells, cellType = "cellType", limit = c(0.2, 5))+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
...
cells |>
  filter(cellsToUse) |>
  filter(region == "region_3") |>
  colTest(feature = "cellType", condition = "Status")


cells |>
  filter(cellsToUse) |>
  filter(region == "region_4") |>
  colTest(feature = "cellType", condition = "Status")
...
```{r}
sort(table(cells[,cells$niche=="region_9"]$cellType))
```
...
cells$cellTypeMergeTumour <- cells |>
  colData() |>
  as.data.frame() |>
  mutate(cellTypeMergeTumour = factor(case_when(
      # cellType == "T_cell_dn" ~ "T_cell",
      # cellType == "T_cell_CD8" ~ "T_cell",
      # cellType == "T_cell_CD4" ~ "T_cell",
      cellType == "CK_low_tumour" ~ "Tumour",
...
stateChangesTcells <- calcStateChanges(
  ...
  assay = "norm",
  cellType = "cellTypeMergeTcells",
  type = "abundancesTcells",
  minCells = 5)


stateMatTcells <- prepMatrix(stateChangesTcells, replaceVal = NA, column = "tval")

stateMatTcells[stateMatTcells==Inf] <- NA
stateMatTcells <- stateMatTcells[,colMeans(is.na(stateMatTcells[rownames(prop),]))<0.6]


stateResultsTcells <- colTest(stateMatTcells[rownames(prop), ], droplevels(clinical[rownames(prop), "Status"]))

stateResultsTcells[grep("T_cell__CK_low_tumour", stateResultsTcells$cluster),]

boxplot(stateMatTcells[rownames(prop), "T_cell__CK_low_tumour__ECAD"]~ droplevels(clinical[rownames(prop), "Status"]))
...
data.frame(colData(cells),reducedDim(cells, "abundancesTcells"), t(assay(cells,  "norm")))|>
  ...
  filter(cellTypeMergeTcells %in% "T_cell", imageID %in% rownames(prop), !is.na(CK_low_tumour)) |>
  mutate(bin = factor(cut(CK_low_tumour,c(0,2,4,6,8,10,100),right = FALSE)))|>
...
data.frame(colData(cells),reducedDim(cells, "distancesTcells"), t(assay(cells, "norm"))) |>
  ...
  filter(cellTypeMergeTcells %in% "T_cell", imageID %in% rownames(prop), !is.na(CK_low_tumour)) |>
  mutate(bin = factor(cut(CK_low_tumour,c(0,50,100,150,200,300,400),right = FALSE))) |>
  filter(!is.na(bin))|>
  group_by(imageID,bin , Status) |>
  summarise(gene = mean(CD8)) |>
  ggplot(aes(x = bin, y = gene, fill = Status)) + geom_boxplot(alpha = 0.5) + 
...
cells <- getDistances(cells,
  cellType = "cellTypeMergeTumour",
  spatialCoords = c("m.cx", "m.cy"),
  redDimName = "distancesTumour"
)
...
cells <- getAbundances(cells,
  ...
  cellType = "cellTypeMergeTumour",
  spatialCoords = c("m.cx", "m.cy"),
  redDimName = "abundancesTumour",
)

stateChangesTumour <- calcStateChanges(
  cells,
  assay = "norm",
  cellType = "cellTypeMergeTumour",
  type = "abundancesTumour",
  minCells = 5)


stateMatTumour <- prepMatrix(stateChangesTumour, replaceVal = NA, column = "tval")

stateMatTumour[stateMatTumour==Inf] <- NA
stateMatTumour <- stateMatTumour[,colMeans(is.na(stateMatTumour[rownames(prop),]))<0.4]


stateResultsTumour <- colTest(stateMatTumour[rownames(prop), ], droplevels(clinical[rownames(prop), "Status"]))

stateResultsTumour[grep("Tumour__T_cell_CD8", stateResultsTumour$cluster),]

boxplot(stateMatTumour[rownames(prop), "Tumour__T_cell_CD8__ECAD"]~ droplevels(clinical[rownames(prop), "Status"]))
...
data <- data.frame(colData(cells),reducedDim(cells, "abundancesTumour"), t(assay(cells,  "norm")))

data |>
  filter(cellTypeMergeTumour %in% "Tumour", imageID %in% rownames(prop), !is.na(T_cell_CD8)) |>
      mutate(bin = factor(cut(T_cell_CD8,c(0,2,4,6,8,10,15,20,30,100),right = FALSE))) |>
  group_by(imageID,bin,Status) |>
  summarise(gene = mean(ECAD)) |>
  ggplot(aes(x = bin, y = gene, fill = Status)) + geom_boxplot()


data <- data.frame(colData(cells),reducedDim(cells, "distancesTumour"), t(assay(cells, "norm")))

data |>
  filter(cellTypeMergeTumour %in% "Tumour", imageID %in% rownames(prop), !is.na(T_cell_CD8)) |>
mutate(bin = factor(cut(T_cell_CD8, c(0,20,50,100,150,200,300,400),right = FALSE))) |>
  group_by(imageID,bin,Status) |>
  summarise(gene = mean(ECAD)) |>
  ggplot(aes(x = bin, y = gene, fill = Status)) + geom_boxplot()
...
data <- data.frame(colData(cells),t(assay(cells,  "norm")))|>
  ...
  group_by(imageID, region, Status) |>
  summarise(gene = mean(CD8))

test = NULL
for(i in unique(data$region)){
test[[i]] <- t.test(gene~Status, data[data$region==i,])
} 
...
data.frame(colData(cells),t(assay(cells,  "norm")))|>
  ...
  group_by(imageID, region, Status) |>
  summarise(gene = mean(CD8)) |>
  ggplot(aes(y=gene,x=region, colour = Status))+
  geom_boxplot()
  

cells$typeRegion <- paste(cells$cellTypeMergeTcells, cells$region, sep = "__")
...
boxplot(data[["gene_mean_celltype"]][rownames(prop),"T_cell__region_4--CD8"]~as.character(clinical[rownames(prop), "Status"]))
