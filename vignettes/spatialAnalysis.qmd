...
# Marker expression within domains
...
geneDomains <- data.frame(colData(cells), t(assay(cells, "norm"))) |>
...
t.test(CD8_avg_exp ~ Status, 
       data = geneDomains |> filter(domain == "region_1"))
```

```{r}
# t-test for region_2 (tumour domain)
t.test(CD8_avg_exp ~ Status, 
       data = geneDomains |> filter(domain == "region_2"))
```
