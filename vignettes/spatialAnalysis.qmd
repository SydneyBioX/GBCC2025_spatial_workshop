...
Finally, we'll subset our samples to include only progressors and non-progressors, so we can compare tissue composition and spatial relationships between these two key subgroups throughout the rest of the workflow.
...
boxplot(
  prop[, "T_cell_CD8"] ~ droplevels(clinical[, "Status"]),
...
The [spicyR](https://www.bioconductor.org/packages/release/bioc/html/spicyR.html) package provides a metric to quantify the degree of localisation or dispersion between two cell types. This metric quantifies whether two cell types in an image tend to be closer together (co-localised) or further apart (dispersed) than would be expected by chance. A positive value indicates that two cell types are co-localised, while a negative value suggests they are dispersed. A value around 0 indicates random distribution. t then tests for changes in this co-localisation metric across different disease states or groups.
...
biologicalHierarchy <- list(T_cell = c("T_cell_CD8", "T_cell_CD4"), Immune = c("B_cell", "T_cell_CD8", "T_cell_CD4",
...
We can then use the `Kontextual` function to evaluate co-localisation between pairs of cell types with respect to a parent population. In this case, we are calculating the `Kontextual` metric over a radius of `r = 100`.
...
### Mixed effects model: interaction between domain and tumour progression status
...
chisq.test(table(cells$cellType, cells$niche))
```
...
We can also visualise T cell proximity to tumour cells vs CD8 expression on a scatterplot.
...
Another approach we can take is to examine the expression of CD8 in the broad spatial domains we identified earlier. We would expect CD8 expression to be higher for progressors vs non-progressions within the tumour domain (`region_2`). 

```{r}
# Load and prepare data: combine cell metadata with normalized gene expression
# Filter for T cells only, and subset to images used in the main analysis
data <- data.frame(colData(cells), t(assay(cells, "norm"))) |>
  ...
  summarise(CD8_avg_exp = mean(CD8), .groups = "drop")

head(data)
```

Next, we'll perform a t-test to check for differences in CD8 expression for each domain between progressors vs non-progressors.

```{r}
# t-test for region_1 (non-tumour domain)
t.test(CD8_avg_exp ~ Status, 
       data = data |> filter(domain == "region_1"))
```

```{r}
# t-test for region_2 (tumour domain)
t.test(CD8_avg_exp ~ Status, 
       data = data |> filter(domain == "region_2"))
```

From the test results above, we can see that the average CD8 expression in the tumour domain (`region_2` is higher in progressors compared to non-progressors. In the non-tumour domain (`region_1`), CD8 expression between progressors and non-progressors is similar.

```{r}
# Visualise CD8 expression by domain and status using boxplots
data |> 
  ggplot(aes(y = CD8_avg_exp, x = domain, colour = Status)) +
  geom_boxplot() +
  labs(title = "CD8 Expression in T Cells by Domain and Status", y = "Mean CD8 Expression", x = "Domain")
```
...
# Add pair-wise associations(l-function metric used in spicyR)
# approx. runtime: 15 secs
...
# Add proportions of each region in each image (lisaClust)
data[["regions"]] <- getProp(cells, "niche")
```

We will also create an outcome vector which represents the predictions to be made. 

```{r}
outcome <- cells$Status[!duplicated(cells$imageID)]
outcome
```
