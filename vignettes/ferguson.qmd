...
save(cells, file = "data/cellsFerguson.rda")
```
...
cells <- cells |>
  mutate(cellType = case_when(
      clusters == "cluster_1" ~ "granulocyte",
      clusters == "cluster_2" ~ "macrophage", # tumour cell type
      clusters == "cluster_3" ~ "myeloid",
      clusters == "cluster_4" ~ "Fibroblast",
      clusters == "cluster_5" ~ "squamous",
      clusters == "cluster_6" ~ "B_cell",
...
cells |>
  ...
  colTest(feature = "cellType", condition = "group")
...
```{r}
tab <- chisq.test(table(cells$cellType, cells$Niche))$res
...
sort(table(cells[,cells$Niche=="region_24"]$cellType))
```
...
treekorParents <- list(T_cell = c("T_cell_CD4", "T_cell_CD4"), Immune = c("T_cell_CD4", "T_cell_CD4", "B_cell", "macrophage", "myeloid","granulocyte"))
...
cells$x <- spatialCoords(cells)[,1]
cells$y <- spatialCoords(cells)[,2]
...
kontextResults <- colTest(kontextMat[rownames(prop),], as.character(clinical[rownames(prop), "group"]))
...
cells$cellType_T_cell <- cells$cellType
cells$cellType_T_cell[cells$cellType_T_cell %in% c("T_cell_CD4", "T_cell_CD8")] <- "T_cell"

cells <- getAbundances(cells,
  r = 50,
  nCores = nCores,
  cellType = "cellType_T_cell",
  #spatialCoords = c("m.cx", "m.cy")
  )
...
results[grep("T_cell__squamous",results$cluster),]
```
